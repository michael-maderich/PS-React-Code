...
var intWithinBounds = function (n, lower, upper) {
    if (n !== Math.floor(n))
        return false; // n is not an integer
    if (n < lower)
        return false;
    if (n >= upper)
        return false;
...
var areaOfCountry = function (name, area) {
    var WORLD_LANDMASS = 148940000;
    var proportional_mass = Math.round((area * 100 * 100 / WORLD_LANDMASS)) / 100;
